...
async function fetchTopics(courseId: string): Promise<ReadonlyTopicsResponse> {
  try {
    ...
    const response = await getAxios(`/api/v0/courses/${courseId}/topics/user/${userId}`)
...
export const Route = createFileRoute('/training/student/all-courses/$courseId/topics/')({  
  beforeLoad: async ({ location }) => {
   return authGuardCheck({ location }, ['ROLE_Student', 'ROLE_Instructor']);
  },
